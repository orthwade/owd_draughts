...
namespace owd
{
	struct s_sound_
	{
		s_sound_(std::shared_ptr<c_sound>& in_s, ALuint in_b);
		s_sound_(std::shared_ptr<c_sound>& in_s, ALuint in_b, ALuint in_c);
		~s_sound_();

		// sound_ptr
		std::shared_ptr<c_sound> s = std::make_shared<c_sound>();
	...
	class c_sound_source : public c_basic_object
	{
	public:
		c_sound_source();
		c_sound_source(std::wstring_view name);
		c_sound_source(std::wstring_view name, std::shared_ptr<c_sound>& sound);

		~c_sound_source();

		void set_position(float x, float y, float z);
		void set_gain(float gain);
		void set_gain(c_sound* sound,			float gain);
		...
		bool enable	(std::shared_ptr<c_sound>& sound_ptr);
		bool pause	(std::shared_ptr<c_sound>& sound_ptr);
		bool stop	(std::shared_ptr<c_sound>& sound_ptr);

		bool enable	(std::vector<std::shared_ptr<c_sound>> vec_sound_ptr);
		bool pause	(std::vector<std::shared_ptr<c_sound>> vec_sound_ptr);
		bool stop	(std::vector<std::shared_ptr<c_sound>> vec_sound_ptr);
		...
		bool add_sound(std::shared_ptr<c_sound>& sound);

		bool remove_sound(std::shared_ptr<c_sound>& sound);
		bool remove_sound(std::vector<c_sound*> vec_sound_ptr);
