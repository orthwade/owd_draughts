...
namespace owd
{
	...
	c_sound_source::c_sound_source(std::wstring_view name, std::shared_ptr<c_sound>& sound)
	{
		m_logger << name << ' ' << "----source constructor START----\n";
	...
	void c_sound_source::set_gain(c_sound* sound, float gain)
	{
		//m_mtx.lock();
		{
	...
	void c_sound_source::set_gain(std::wstring_view name, float gain)
	{
		...
		//m_mtx.lock();
		{
			...
			if (m_vec_sounds.size() > 0)
			{
				...
				for (uint16_t i = 0; i != m_vec_sounds.size(); ++i)
				{
					if (m_vec_sounds.at(i).s->name() == name_str)
					{
						AL_CALL(alSourcef((m_vec_sounds.at(i).c), AL_GAIN, gain));
						found = true;
						break;
	...
	void c_sound_source::set_velocity(float x, float y, float z)
	{
		//m_mtx.lock();
		{
	...
	xyz_t c_sound_source::get_position()
	{
		xyz_t result{};
	...
	float c_sound_source::get_gain(c_sound* sound)
	{
		float result{};
	...
	float c_sound_source::get_gain(std::wstring_view name)
	{
		float result{};
		...
		//m_mtx.lock();
		{
			...
			for (uint16_t i = 0; i != m_vec_sounds.size(); ++i)
			{
				if (m_vec_sounds.at(i).s->name() == name)
				{
					AL_CALL(alGetSourcef((m_vec_sounds.at(i).c), AL_GAIN, &result));
					found = true;
					break;
	...
	bool c_sound_source::enable(std::shared_ptr<c_sound>& sound_ptr)
	{
		bool result = true;
	...
	bool c_sound_source::pause(std::shared_ptr<c_sound>& sound_ptr)
	{
		bool result = true;
	...
	bool c_sound_source::stop(std::shared_ptr<c_sound>& sound_ptr)
	{
		bool result = true;
	...
	bool c_sound_source::enable(std::vector<std::shared_ptr<c_sound>> vec_sound_ptr)
	{
		bool result = true;
	...
	bool c_sound_source::pause(std::vector<std::shared_ptr<c_sound>> vec_sound_ptr)
	{
		bool result = true;
	...
	bool c_sound_source::stop(std::vector<std::shared_ptr<c_sound>> vec_sound_ptr)
	{
		bool result = true;
	...
	bool c_sound_source::enable_by_name(std::wstring_view name)
	{
		bool result = true;
	...
	bool c_sound_source::pause_by_name(std::wstring_view name)
	{
		bool result = true;
	...
	bool c_sound_source::stop_by_name(std::wstring_view name)
	{
		bool result = true;
	...
	bool c_sound_source::enable_by_name(std::vector<std::wstring_view> vec_names)
	{
		bool result = true;
			...
			if (m_vec_sounds.size() > 0)
			{
				if (vec_names.size() > 0)
				{
					...
					for (uint16_t n = 0; n != vec_names.size(); ++n)
					{
						...
						for (uint16_t i = 0; i != m_vec_sounds.size(); ++i)
						{
							if (m_vec_sounds.at(i).s->name() == vec_names.at(n))
							{
								found = true;
								AL_CALL(alSourcePlay((m_vec_sounds.at(i).c)));
								break;
	...
	bool c_sound_source::pause_by_name(std::vector<std::wstring_view> vec_names)
	{
		bool result = true;
	...
	bool c_sound_source::stop_by_name(std::vector<std::wstring_view> vec_names)
	{
		bool result = true;
	...
	bool c_sound_source::add_sound(std::shared_ptr<c_sound>& sound)
	{
		bool result = false;
		//m_mtx.lock();
		{
	...
	bool c_sound_source::remove_sound(std::shared_ptr<c_sound>& sound)
	{
		bool result = false;
		//m_mtx.lock();
		{
	...
	bool c_sound_source::remove_sound(std::vector<c_sound*> vec_sound_ptr)
	{
		bool result = true;
		bool found = false;
	...
	bool c_sound_source::remove_sound_by_name(std::wstring_view name)
	{
		bool result = false;
		//m_mtx.lock();
		{
	...
	bool c_sound_source::remove_sound_by_name(std::vector<std::wstring_view> vec_names)
	{
		bool result = true;
		bool found = false;
	...
	void c_sound_source::detach_all_sounds()
	{
		//m_mtx.lock();
		{
	...
	s_sound_::s_sound_(std::shared_ptr<c_sound>& in_s, ALuint in_b)
	...
	s_sound_::s_sound_(std::shared_ptr<c_sound>& in_s, ALuint in_b, ALuint in_c)
		:
		s(in_s),
		b(in_b),
		c(in_c)
